...
> = {
  ...
  contactor: {
    ...
    role: "Силовой коммутатор с катушкой управления: включает и отключает мощные нагрузки по сигналу (таймер, реле, автоматика).",
    benefit: "Нужен для управления отоплением, насосами, освещением и другими нагрузками без ручного рычага автомата.",
  },
  socket: {
    title: "Розетка на DIN",
    role: "Щитовая розетка для подключения инструментов или временной нагрузки прямо из щитка.",
    benefit: "Удобна для обслуживания щита, но должна быть защищена автоматом и по возможности УЗО.",
  },
};

export const panelGuideDisclaimer =
  "Током показывает, какие приборы видны на фото и в схеме. Сервис не проверяет, правильно ли они соединены между собой, подобраны ли номиналы и исправно ли всё работает на самом деле. Окончательную оценку может дать только очный осмотр электрика.";
