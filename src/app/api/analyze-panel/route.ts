...
- breaker — автоматический выключатель (любой: вводной или групповой — НЕ пытайся угадать вводной)
...
Правила:
- Никогда не ставь type=main_breaker. Вводной автомат пользователь подтвердит позже диагностикой. Все автоматы — type=breaker.
...
- name: краткое русское имя («Автомат», «УЗО», «Дифавтомат»). Не пиши «Вводной». Не выдумывай комнаты, если на щитке нет подписей.
- status: "verified" только если уверенность ≥ 95 (почти наверняка), иначе "pending", если почти не видно — "unknown".
- confidence: 0–100. Будьте консервативны: тип, бренд и номинал указывайте только при высокой уверенности.
- manufacturer / brandKey: заполняйте только при уверенности ≥ 95. Иначе опустите поля.
- series: серия прибора с корпуса (например S201, ВА47-29, iC60N, ВД1-63, OptiDin BM63). Указывай только если на лицевой панели рядом с логотипом производителя реально читается название серии. Если на корпусе только логотип бренда без серии — опусти поле series. Не выдумывай.
- rating, poles, modules: заполняйте только при уверенности ≥ 95. Если номинал не читается — rating="—".
...
- linesCount: число линий нагрузки (breaker + diff_breaker), без шин.
